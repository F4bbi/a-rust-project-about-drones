...
export const useToolbarStore = create<ToolbarState>((set) => ({
  ...
  setSelectedSpecificNode: (node: NodeData | null) =>
    set({ selectedSpecificNode: node }),
  setAvailableNodes: (nodes: NodeData[]) => set({ availableNodes: nodes }),
  addCreatedNode: (node: CreatedNode) =>
    set((state) => ({ createdNodes: [...state.createdNodes, node] })),
  resetToolbar: () =>
    set({
      activeTool: "cursor",
      selectedNodeType: null,
  ...
  // Message-related actions
  setSelectedMessageType: (messageType: string | null) =>
    set({ selectedMessageType: messageType }),
  setMessageFormData: (data: any) => set({ messageFormData: data }),
  setIsSelectingNodes: (selecting: boolean) =>
    set({ isSelectingNodes: selecting }),
  setSelectedFromNode: (nodeId: string | null) =>
    set({ selectedFromNode: nodeId }),
  resetMessageState: () =>
    set({
      selectedMessageType: null,
      messageFormData: {},
