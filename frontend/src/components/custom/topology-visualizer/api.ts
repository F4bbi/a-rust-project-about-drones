export const getBackendNodeType = (nodeType: string, nodeName: string) => {
  switch (nodeType) {
    case 'drone':
      return {
        node_type: 'drone',
...
export const createNode = async (nodeType: string, nodeName: string) => {
...
export const createEdge = async (fromId: string, toId: string) => {
  ...
  if (!response.ok) throw new Error(`Failed to create edge: ${response.statusText}`)
  const data = await response.json()
  return {
    success: true,
